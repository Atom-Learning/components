...
type TCardsItemProps = { image: string, heading: string, description: string, link: { href: string, isExternal: boolean }, tags: string | string[] }

const CardsItem: React.FC<TCardsItemProps> = ({ image, heading, description, link }) => {
...
export const Cards: React.FC<CardsProps> = ({ showSearch, showTagsFilter, items }) => {
  ...
  const tags = React.useMemo(() => {
    const newTags = [];
    const pushToTags = (tag) => { if (!newTags.includes(tag)) newTags.push(tag) }
    items.forEach(({ tags }) => {
      if (typeof tags === 'string') { pushToTags(tags) }
      else if (Array.isArray(tags)) { tags.forEach(pushToTags) }
    })
    return newTags
  }, [items])
  ...
  return (
    <Stack gap={4} direction="column">
      ...
      <Grid
        ...
        {items.map((props) => {
          if (!checkIfMatchingTags(props.tags, selectedTags) || !checkIfMatchingSearch(props, searchValue)) return null;
          return (
