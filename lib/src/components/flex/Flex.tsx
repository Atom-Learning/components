...
const createVariants = <T extends GlobalValue[]>(variants: T, fn: (string: T[number]) => CSS) => {
  return variants.reduce((prev, variant) => ({ ...prev, [variant]: fn(variant) }), {} as Record<T[number], CSS>)
}

const globalValues = ['inherit', 'initial', 'revert', 'revert-layer', 'unset'] as const
...
const StyledFlex = styled('div', {
  ...
  variants: {
    ...
    wrap: createVariants([...globalValues, 'nowrap', 'wrap', 'wrap-reverse'], (v) => { return { flexWrap: v } }),
    ...
    justify: createVariants([...globalValues, 'normal', 'unsafe', 'safe', 'start', 'center', 'end', 'flex-start', 'flex-end', 'left', 'right', 'space-between', 'space-around', 'space-evenly', 'stretch'], (v) => { return { justifyContent: v } }),
    align: createVariants([...globalValues, 'normal', 'unsafe', 'safe', 'center', 'start', 'end', 'self-start', 'self-end', 'flex-start', 'flex-end', 'baseline', 'first baseline', 'last baseline', 'stretch'], (v) => { return { alignItems: v } }),
