...
## Customizable Dropdown Trigger

NavigationMenu.Dropdown gives you the way to exchange the default Trigger text with a custom component.
Be aware that 'title' property is required in any case to satisfy underlying operations.

```tsx preview
<NavigationMenu.Dropdown title="User" triggerComponent={<Avatar />}>
  // content
</NavigationMenu.Dropdown>
```

If you need, you can also modify the CSS
