...
- `Sidedrawer.Trigger` - It's the main `trigger element` responsible for opening the whole `Sidedrawer.Content`. This component is build in top of [radix dialog trigger](https://www.radix-ui.com/docs/primitives/components/dialog#trigger)
- `Sidedrawer.Content` - This is whe wrapper for whole content (`Sidedrawer.Header`, `Sidedrawer.Body`, `Sidedrawer.Footer` and other nested elements). This component is build in top of [radix dialog content](https://www.radix-ui.com/docs/primitives/components/dialog#content)
