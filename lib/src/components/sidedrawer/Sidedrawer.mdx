...
`Sidedrawer` has a controlled state, that's why it requires `isOpen` and `onClose` properties.

- `isOpen` property determines if the whole `Sidedrawer` is visible or not.
- `onClose` prop should be a function which updates `isOpen` state. This function is triggered automatically when click on `Sidedrawer.Overlay` or press `Escape` key.
...
- `Sidedrawer.Overlay` - Full screen overlay positioned under the `Sidedrawer menu`. It inherits the `onClose` prop from `Sidedrawer`, so you can close the whole menu by clicking on `Overlay` area. This in an optional element.
