...
`DataTable` has two different roots namely; `DataTable` and `DataTable.Remote`. Both components manages the table's state and exposes it via the React Context API. This state can be accessed by any child components by calling `useDataTable`.
