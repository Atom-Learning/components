---
title: DataTable
component: DataTable, DataTable.Remote, DataTable.Head, DataTable.HeaderCell, DataTable.Body, DataTable.Row, DataTable.DataCell, DataTable.GlobalFilter, DataTable.Remote, DataTable.Loading, DataTable.Error
...
`DataTable` has two different roots namely; `DataTable` and `DataTable.Remote`. Both components manages the table's state and exposes it via the React Context API. This state can be accessed by any child components by calling `useDataTable`. You can pass an `initialState` prop to `DataTable` as described in the [`@tanstack/react-table` docs](https://tanstack.com/table/v8/docs/api/core/table#initialstate).
`DataTable.Remote` is used when you want achieve server-side pagination and sorting.
...
`DataTable.Remote` accepts a `fetcher` function that is used to fetch the paginated data. The `fetcher` function provides you with the necessary parameters to query your api endpoint; `pageIndex`, `pageSize`, `sortBy` and `sortDirection`. The response from the `fetcher` function must match the following schema:

```ts
{
  results: Array<Record<string, unknown>> // your paginated data
  total: number // the total number of elements in your data
}
```

A loading state using `<DataTable.Loading>` is automatically included in `DataTable.Remote` which is visible while the `fetcher` promise is pending. You can use `DataTable.Error` to display your own error component when the `fetcher` function promise rejects. It provides a `retry` function, allowing the children of the component to recall `fetcher` with the current pagination and sorting parameters.
...
<DataTable.Remote
  ...
  fetcher={async (pageIndex, pageSize, sortBy, sortDirection) => {
    const nextPage = pageIndex + 1
    const data = await fetch(
      `https://your-api?page=${nextPage}&pageSize=${pageSize}&order=${sortBy}&dir=${sortDirection}`
    )
...
>
  ...
  <DataTable.Error>
    {(retry) => {
      return (
        <>
          <InlineMessage>Error occured</InlineMessage>
