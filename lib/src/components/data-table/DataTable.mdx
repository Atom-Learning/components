...
The `DataTable.DragAndDropContainer` can be rendered as a child of `DataTable` and parent of `DataTable.Table` to allow users to reorder table rows via drag and drop. It takes an optional `onChange` prop which is a function that fires when rows have been re-ordered via drag-and-drop. Use this to sync those changes with external data sources.
...
<DataTable data={data} columns={columns}>
  <DataTable.DragAndDropContainer idColumn="userId">
    <DataTable.Table />
  </ DataTable.DragAndDropContainer>
</DataTable>
```
