---
title: DataTable
component: DataTable, DataTable.Head, DataTable.HeaderCell, DataTable.Body, DataTable.Row, DataTable.DataCell, DataTable.GlobalFilter, DataTable.Loading, DataTable.Error
...
`DataTable` accepts an `asyncData` function that is used to fetch the paginated data. The `asyncData` function provides you with the necessary parameters to query your api endpoint; `pageIndex`, `pageSize`, `sortBy` and `sortDirection`. The response from the `asyncData` function must match the following schema:
...
A loading state using `<DataTable.Loading>` is automatically included in `DataTable` which is visible while the `asyncData` promise is pending. You can use `DataTable.Error` to display your own error component when the `asyncData` function promise rejects. It provides a `retry` function, allowing the children of the component to recall the `asyncData` function with either the last pagination options or your own custom pagination properties
