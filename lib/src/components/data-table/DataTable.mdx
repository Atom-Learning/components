...
`DataTable` can be used with local pagination or server-side pagination (getting only the data needed for the current page). To use one or the other, you have to use the `data` or `asyncData` prop respectively.

The `asyncData` function accepts an object with the necessary parameters to get the relevant piece of data for the current page and order. All the parameters are optional, with these defaults:
...
The response from the `asyncData` function must match the following schema:
...
A loading state using `<DataTable.Loading>` is automatically included in `DataTable` which is visible while the `asyncData` promise is pending.

You can use `DataTable.Error` to display your own error component when the `asyncData` function promise rejects. Notice `DataTable.Error` doesn't render anything on its own, but whatever is passed as childern. It provides a `retry` function, allowing the children of the component to recall the `asyncData` function with either the last pagination options or your own custom pagination properties.
...
<DataTable
  ...
  asyncData={async ({ pageIndex, pageSize, sortBy, sortDirection }) => {
    ...
    {(retry) => {
      return (
        <>
          ...
          <Button onClick={() => retry?.({ pageIndex: 3, pageSize: 10 })}>
            Retry
          </Button>
        </>
      )
