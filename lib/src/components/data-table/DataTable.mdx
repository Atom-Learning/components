...
`DataTable` accepts a `fetcher` function that is used to fetch the paginated data. The `fetcher` function provides you with the necessary parameters to query your api endpoint; `pageIndex`, `pageSize`, `sortBy` and `sortDirection`. The response from the `fetcher` function must match the following schema:
...
A loading state using `<DataTable.Loading>` is automatically included in `DataTable` which is visible while the `fetcher` promise is pending. You can use `DataTable.Error` to display your own error component when the `fetcher` function promise rejects. It provides a `retry` function, allowing the children of the component to recall the `fetcher` function with either the last pagination options or your own custom pagination properties
