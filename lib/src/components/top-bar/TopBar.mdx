...
By default, `TopBar` uses a flex layout with flex items placed immediatley after each other. To align flex items, an optional css prop with `justifyContent` can be passed.
