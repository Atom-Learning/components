...
describe(`KeyboardShortcut component`, () => {
  it('renders', async () => {
    const { container } = render(
      ...
      >
        <KeyboardShortcut.Indicator>Meta + A</KeyboardShortcut.Indicator>
      </KeyboardShortcut>
    )
  ...
  it('works when targeting window', async () => {
    ...
    expect(onKeyboardShortcutMock).toHaveBeenCalledWith(
      expect.objectContaining({ shortcut: { key: 'a' } })
    )
  })
  ...
  it('works when targeting element', async () => {
    ...
    const keyboardCaptureBox = screen.getByTestId(
      'keyboard-shortcut-capture-box'
    )
