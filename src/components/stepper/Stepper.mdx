...
The root `Stepper` component must be passed `stepCount` as a number. It can also take `onComplete` as a function that gets called when the user clicks the `StepForward` button while on the last step.

```tsx preview
<Stepper stepCount={3} onComplete={() => console.log('Finished')}>
...
```tsx preview
<Stepper stepCount={3} allowSkip onComplete={() => console.log('Finished')}>
  <Stepper.StepBack label={(activeStep) => 'Back'} />
  <Stepper.Steps />
  <Stepper.StepForward
    label={(activeStep) => (activeStep === 2 ? 'Finish' : 'Next')}
