...
export const Editor = () => {
    ...
    const decorate = useCallback(([node, path]) => {
        ...
        // const tokens  = fromMarkdown(node.text, {
        // extensions: [syntax()],
        ...
        console.log({ nt: node.text, tokens, ranges })

        return ranges
    }, [])

    return (
        <Slate editor={editor} value={initialValue} onChange={(e) => console.log(e)}>
            {/* <MarkdownContent> */}
            <Editable
                decorate={decorate}
...
const Leaf = ({ attributes, children, leaf }) => {
    ...
    console.log({children, leaf})
    if (leaf.heading) return <h2 {...attributes}>{children}</h2>
    if (leaf.title) return <MarkdownContent content={children} {...attributes} />
    return (
        <span
            {...attributes}
        //   className={css`
