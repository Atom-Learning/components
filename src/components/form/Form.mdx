...
deciding dynamically whether to make a field required. If it will also be required, you can pass a `string` containing the error message instead. The following two inputs are equivalent:
...
If you pass one of those properties the `value` directly, you'll get a default error message:

```tsx center
<Form>
  <InputField type="number" name="numCats" label="Number of cats" validation={{min: 3}/> // The minimum allowed value is 3
  <InputField name="faveName" label="Favourite cat name" validation={{maxLength: 20}} /> // the maximum allowed length is 20
  <Button>Submit</Button>
</Form>
```
