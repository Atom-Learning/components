...
If you do want to add default validation, pass something like this: `register?.({ maxLength: {value: 2, message: "short data only!" },...validation })`. Note that optional chaining is
important here to protect from runtime errors if your field gets rendered outside of a `Form`.
...
```tsx
const EmailField = ({ register, error, validation, ...rest }) => {
  return (
    <InputField
      css={{ mb: '$3' }}
      autoComplete="email"
