...
- For the mapping to work correctly, instead of using the index from a `map()` or some other id, it is required to pass ids generated by the `useFieldArray` hook.- Each dynamic field needs to take a default value from the fields object
