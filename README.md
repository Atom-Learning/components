...
Different tools are used to validate the components as each serves a different purpose. The a11y storybook addon will validate all the stories and explain each violation, whereas the accessibility unit tests will fail the pipeline and prevent releasing inaccessible components. **However, these tests are only valid with the context they are testing on, so they can't guarantee 100% that a component is fully accessible.** Therefore, please add stories for all the variations a component has. Also, there is a need for manual testing and design validation.
